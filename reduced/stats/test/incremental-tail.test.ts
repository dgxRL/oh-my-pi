...
import { closeDb, getFileOffset, getOverallStats, getRecentRequests, initDb } from "../src/db";
...
describe("incremental stats ingestion", () => {
	for (const kind of ["message", "tool", "tool-entry"] as const) {
		const title =
			kind === "tool-entry"
		...
		it(title, async () => {
			...
			await syncAllSessions({ workers: 1 });
			const table = kind === "message" ? "messages" : "tool_calls";
	...
	it("reads only appended bytes after reopening the database", async () => {
	...
	for (const operation of ["replace", "truncate"] as const) {
		it(`rebuilds derived totals after ${operation} instead of reusing the old offset`, async () => {
			const file = await session(
	...
	it("upgrades offset-only databases without losing incremental state", async () => {
	...
	it("carries worker syncs across database restarts", async () => {
		const file = await session(tier("priority") + assistant("first"));
		await syncAllSessions({ workers: 2 });
		closeDb();
	...
	it("retries an incomplete final entry on the next pass", async () => {
