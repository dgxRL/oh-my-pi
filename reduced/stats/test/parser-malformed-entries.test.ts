...
import { getSessionsDir, getStatsDbPath } from "../src/paths";
...
describe("legacy entries without a recorded price", () => {
	it("estimates the stored cost at the request timestamp instead of freezing zero", async () => {
		...
		// 1M total tokens at the flat $3.00/M rate.
		expect(stored.find(request => request.entryId === "unpriced")?.usage.cost?.total).toBeCloseTo(3.0, 8);
		...
		expect(getOverallStats().totalCost).toBeCloseTo(3.0, 8);

		closeDb();
		await initDb();

		const reopened = getRecentRequests(2);
		expect(reopened.find(request => request.entryId === "unpriced")?.usage.cost?.total).toBeCloseTo(3.0, 8);
		expect(reopened.find(request => request.entryId === "explicit-zero")?.usage.cost?.total).toBe(0);
		expect(getOverallStats().totalCost).toBeCloseTo(3.0, 8);
	});

	it("bills zero-sentinel usage at the flat rate (no scheduling concept)", async () => {
		const file = await writeSession([
			JSON.stringify({
				type: "message",
				id: "no-timestamp",
		...
		// No scheduling concept in the reduced copy: bucket tokens bill at the
		// flat rate even when the message timestamp is the zero sentinel.
		expect(getRecentRequests(1)[0]?.usage.cost?.total).toBeCloseTo(3.0, 8);
		expect(getOverallStats().totalCost).toBeCloseTo(3.0, 8);
	});
	...
	it("recovers the entry timestamp when the message timestamp is the zero sentinel", async () => {
		...
		// Recovered entry time keeps the request real; flat rate applies.
		expect(getRecentRequests(1)[0]?.usage.cost?.total).toBeCloseTo(3.0, 8);
		expect(getOverallStats().totalCost).toBeCloseTo(3.0, 8);
	});
