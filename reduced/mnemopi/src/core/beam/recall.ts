/**
 * Reduced port of packages/mnemopi/src/core/beam/recall.ts.
 * Hybrid recall: FTS5 ranks (normalized) + optional dense cosine over stored
 * JSON embeddings + lexical group scoring + importance + recency/temporal decay
 * + veracity/tier weights, with MMR diversification and fact-table fusion.
 * Dropped vs the original: polyphonic recall voices, query cache, recall
 * diagnostics, native vector kernels (dense path always uses the exact
...
/**
 * Default per-result content preview cap enforced by `recall`. Content longer
 * than this is clipped and the last character replaced with `…`; the full row
 * remains reachable via `Mnemopi.get()`.
 */
