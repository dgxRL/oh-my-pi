/** Reduced port of packages/mnemopi/src/core/beam/types.ts — types the reduced workflow needs. */
...
export interface BeamMemoryState {
	...
	caches: BeamCaches;
	config: BeamConfig;
	/** Tracks in-flight background tasks scheduled by `remember(..., { extract: true })`. Dropped in reduced. */
...
export interface RememberOptions {
	...
	authorType?: string | null;
	channelId?: string | null;
	/** Enables proactive episodic-graph linking for this memory. */
...
export interface RecallOptions {
	...
	mmrLambda?: number;
	/**
	 * Maximum characters of `content` returned per {@link RecallResult}. Longer
	 * content is clipped with a trailing `…`; `0` disables clipping. Defaults
	 * to {@link RECALL_CONTENT_PREVIEW_CHARS} (500).
	 */
	contentPreviewChars?: number;
}

export interface RecallEnhancedOptions extends RecallOptions {
	/** Accepted for API compatibility; the reduced copy never caches. */
	useCache?: boolean;
...
export type RecallResult = RecallRowFields & {
	...
	id: string;
	content: string;
	/** True when {@link content} is a clipped preview of the stored row. */
	truncated?: boolean;
