// Ported from packages/mnemopi/src/core/veracity-consolidation.ts
	...
	resolveConflict(conflictId: number, winningFactId: string): void {
		this.serializedWrite(() => {
			...
			if (conflict.resolution !== null) {
				console.warn(`resolve_conflict: conflict ${conflictId} already resolved; ignoring re-resolution`);
				return;
			}
			let losingId: string;
			if (winningFactId === conflict.fact_a_id) losingId = conflict.fact_b_id;
				...
				console.warn(`resolve_conflict: winning_fact_id ${winningFactId} matches neither fact; declining`);
				return;
			}
			const now = nowIso();
			this.conn
