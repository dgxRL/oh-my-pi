// Ported from packages/mnemopi/src/core/cost-log.ts
...
export function getConn(dbPath?: string): Database {
	const target = dbPath ?? DEFAULT_LOG_DB;
	return openDatabase(target, { pragmas: false });
}
	...
	try {
		conn
			.query(`
				...
				VALUES (?, ?, ?, ?, ?, ?)
			`)
			.run(sessionId, memoryCount, tokenCount, estimatedCostUsd, model, new Date(Date.now() - new Date().getTimezoneOffset() * 60_000).toISOString().replace("Z", ""));
	} finally {
		conn.close();
	}
}
